...
namespace C____RPG
{
    ...
    public sealed partial class GamePage : Page
    {
        ...
        private DispatcherTimer timePlayedTimer;
        ...
        public GamePage()
        {
            ...
            //TESTING: THIS WILL GIVE YOU LOTS OF RESOURCES FOR TESTING PURPOSES
            //This will also trigger the first mission instantly.
            /*
            ...
            */
            ...
            _game.GetPlayer().GetInventory().AddItems(_game.getSource("woodcutting").resource, 0);
            _game.GetPlayer().GetInventory().AddItems(_game.getSource("mining").resource, 0);
            _game.GetPlayer().GetInventory().AddItems(_game.getSource("fishing").resource, 0);


        }

        //Event for the sell button in the shop
        ...
        //Actually the update loop that updates each second to update important gameplay features.
        private void secondeTick(object sender, object e)
        {
            //gets player stats from dictionary 
            var stats = _game.GetPlayer().GetStats();
            timeplayed.Text = "Time played: " + stats["totalplaytime"] + " sec";

            //Displays home options button when player is at home.
            ...
            //Gets the current money from the player
            inventory_money.Text = "" +  _game.GetPlayer().coins;

            //Checks if the player can buy an upgrade. if not, it will be red or when it reached its limit it will be blue.
            ...
            //Updates the story when a mission is completed 
            var story = _game.GetPlayer().getStory();
            ...
            if (story.GetCurrentMission() != null)
            {
                ...
                //Activates when mission is completed
                if (mission_progress > mission_toGo)
                {
                    ...
                    _game.GetPlayer().ChangeCoins(story.GetCurrentMission().reward , true);
                    ...
                    //activated when there are not extra missions anymore
                    if (story.GetCurrentMission() == null)
                    {
        ...
        //Triggers when the player reached a skillspot and activates each second
        private async void XpEvent(object sender, object e)
        {
            ...
            //Gets the playerlocation and sets the xp counter to it.
        ...
        {
            //stops all audio to dissolve a bug and stops the act
            ...
            //Creates a storyboard to let the player move
            ...
            //Stops and clears the story to be sure
            Moving_object.Stop();
